<?php 
	...
	{	
		...
		{
			$result = $dbh->prepare("SELECT * FROM STUDENTS WHERE STUDENT_ID = :userid");
			$result->bindParam(':userid',$userid);
			$result->execute();
			$row = $result->fetch(PDO::FETCH_ASSOC);
			if (password_verify($oldPassword, $row['STUDENT_PASSWORD'])) {
				if ($newPassword == $confirmPassword) {

					$hashPassword = password_hash($newPassword,PASSWORD_BCRYPT,$options);

					$query = "UPDATE STUDENTS SET STUDENT_PASSWORD = :newPassword WHERE STUDENT_ID = :user_id";
